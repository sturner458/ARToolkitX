...
bool ARController::updateWithImage(ARUint8 *image, bool lowRes, bool doDatums, bool doMapper, int markerType, int numberOfDatums)
{
    ...
    if (doSquareMarkerDetection) {
        ...
        m_squareTracker->update(image0, image1, m_trackables, lowRes, doDatums, doMapper, markerType, numberOfDatums);
    }
#if HAVE_NFT
    if (doNFTMarkerDetection) {
        if (!m_nftTracker->isRunning()) {
