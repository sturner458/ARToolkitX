...
void Mapper::Update2(ARMultiMarkerInfoT* map, ARMultiMarkerInfoT* map2, std::vector<ARTrackable *>& trackables) const {
    ...
    for (const int uid : all_uids_) {
        ...
        for (std::vector<ARTrackable *>::iterator it = trackables.begin(); it != trackables.end(); ++it) {
            ...
            if ((*it)->type == ARTrackable::SINGLE) {
                if (((ARTrackableSquare *)(*it))->UID == uid) {
                    arMultiAddOrUpdateSubmarker(map2, uid, AR_MULTI_PATTERN_TYPE_MATRIX, width_, mTrans, 0);
                    break;
                }
            } else if ((*it)->type == ARTrackable::MULTI) {
                if (((ARTrackableSquare *)(*it))->UID == uid) {
                    arMultiAddOrUpdateSubmarker2(map, map2, ((ARTrackableMultiSquare *)(*it))->config, uid, AR_MULTI_PATTERN_TYPE_MATRIX, width_, mTrans, 0);
                    break;
                }
            }
        }
